...
2. **Nome**: escolha um nome descritivo, ex: `ezsp-Delegated`.
...
- O token de acesso e o refresh token são armazenados em cache em `%LOCALAPPDATA%\ezsp\token_cache.json`. Nas execuções seguintes, o token é renovado silenciosamente sem abrir o navegador (o cache dura enquanto o refresh token for válido, tipicamente ~90 dias de inatividade).
